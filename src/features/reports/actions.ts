...
export async function generateReport(formData: FormData) {
  ...
  const team = await db.team.findFirst({ where: { id: data.teamId, userId: user.id }, include: { rosterPlayers: { include: { projections: { where: { season: data.season, week: data.week }, orderBy: { updatedAt: "desc" }, take: 1 } } }, waiverRecommendations: { where: { season: data.season, week: data.week }, orderBy: { priorityScore: "desc" }, take: 3 } } });
  ...
  const ratings = positions.map((position) => { const rows = projected.filter((player) => player.position === position); return { position, value: rows.length ? rows.reduce((sum, player) => sum + player.projections[0].projectedPoints, 0) / rows.length : 0 }; }).sort((a, b) => b.value - a.value);
  ...
  const reportData = { grade, strongestPosition: ratings[0].position, weakestPosition: ratings.at(-1)!.position, riskiestStarter: riskiest?.name, bestBenchOption: bench?.name };
  const facts = { team: team.name, ...reportData, waivers: team.waiverRecommendations.map((item) => (item.addPlayer as { name: string }).name) };
  const fallback = `${team.name} earns a ${grade}. ${ratings[0].position} is the strongest position, while ${ratings.at(-1)!.position} needs the most attention.${bench ? ` ${bench.name} is the best bench option.` : ""}`;
